...
static void modify_callback(char** line, int* cursorPosition, void* ud) {
	char* s = *line;
	char* p = strchr( s, '*' );
	if ( p ) {
...
static void completionHook(char const* context, replxx_completions* lc, int* contextLen, void* ud) {
...
static void hintHook(char const* context, replxx_hints* lc, int* contextLen, ReplxxColor* c, void* ud) {
	char** examples = (char**)( ud );
	int i;
	int utf8ContextLen = context_len( context );
	int prefixLen = (int)strlen( context ) - utf8ContextLen;
...
static ReplxxActionResult word_eater( int ignored, void* ud ) {
	Replxx* replxx = (Replxx*)ud;
	return ( replxx_invoke( replxx, REPLXX_ACTION_KILL_TO_BEGINING_OF_WORD, 0 ) );
}

static ReplxxActionResult upper_case_line( int ignored, void* ud ) {
	Replxx* replxx = (Replxx*)ud;
	ReplxxState state;
...
static char const* recode( char* s ) {
	char const* r = s;
	while ( *s ) {
		if ( *s == '~' ) {
			*s = '\n';
...
static void split( char* str_, char** data_, int size_ ) {
	int i = 0;
	char* p = str_, *o = p;
	while ( i < size_ ) {
		int last = *p == 0;
