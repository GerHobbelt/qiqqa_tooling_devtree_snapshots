...
// prototypes
static Replxx::completions_t hook_completion(std::string const& context, int& contextLen, std::vector<std::string> const& user_data, bool);
static Replxx::hints_t hook_hint(std::string const& context, int& contextLen, Replxx::Color& color, std::vector<std::string> const& user_data, bool);
...
static void hook_color( std::string const& str, Replxx::colors_t& colors, syntax_highlight_t const&, keyword_highlight_t const& );
static void hook_modify( std::string& line, int& cursorPosition, Replxx* );

static bool eq( std::string const& l, std::string const& r, int s, bool ic ) {
...
static Replxx::completions_t hook_completion(std::string const& context, int& contextLen, std::vector<std::string> const& examples, bool ignoreCase) {
...
static Replxx::hints_t hook_hint(std::string const& context, int& contextLen, Replxx::Color& color, std::vector<std::string> const& examples, bool ignoreCase) {
...
static inline bool is_kw( char ch ) {
	return isalnum( ch ) || ( ch == '_' );
}

static void hook_color( std::string const& context, Replxx::colors_t& colors, syntax_highlight_t const& regex_color, keyword_highlight_t const& word_color ) {
...
static void hook_modify( std::string& currentInput_, int&, Replxx* rx ) {
	char prompt[64];
	snprintf( prompt, 64, "\x1b[1;32mreplxx\x1b[0m[%lu]> ", (unsigned long)currentInput_.length() );
	rx->set_prompt( prompt );
}

static Replxx::ACTION_RESULT message( Replxx& replxx, std::string s, char32_t ) {
