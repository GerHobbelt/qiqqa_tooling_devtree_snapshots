...
XtResource clientResources[] = {
    ...
    {"gamma", "Gamma", XtRFloat, sizeof(float), XtOffset(AppDataPtr, gamma),
     XtRString, (XtPointer)TIFF_GAMMA},
...
Arg formArgs[] = {{XtNresizable, True}};

Arg listArgs[] = {
    {XtNresizable, False},         {XtNborderWidth, 0},
    {XtNdefaultColumns, 3},        {XtNforceColumns, True},
    {XtNlist, (int)buttonStrings}, {XtNnumberStrings, XtNumber(buttonStrings)},
    {XtNtop, XtChainTop},          {XtNleft, XtChainLeft},
    {XtNbottom, XtChainTop},       {XtNright, XtChainLeft}};

Arg labelArgs[] = {{XtNresizable, False},   {XtNwidth, 200},
                   {XtNborderWidth, 0},     {XtNjustify, XtJustifyLeft},
                   {XtNtop, XtChainTop},    {XtNleft, XtChainLeft},
                   {XtNbottom, XtChainTop}, {XtNright, XtChainLeft}};

Arg imageArgs[] = {{XtNresizable, True},    {XtNborderWidth, 0},
                   {XtNtop, XtChainTop},    {XtNleft, XtChainLeft},
                   {XtNbottom, XtChainTop}, {XtNright, XtChainLeft}};

XtActionsRec actionsTable[] = {{"quit", QuitProc},
                               {"next", NextProc},
...
uint16_t tfBitsPerSample, tfSamplesPerPixel, tfPlanarConfiguration,
    tfPhotometricInterpretation, tfGrayResponseUnit, tfImageDepth,
    tfBytesPerRow;
int tfDirectory = 0, tfMultiPage = False;
double tfUnitMap,
    tfGrayResponseUnitMap[] = {-1, -10, -100, -1000, -10000, -100000};
 ...
 */
uint16_t *redMap = NULL, *greenMap = NULL, *blueMap = NULL, *grayMap = NULL,
         colormapSize;
char *imageMemory;
char *fileName;
...
int main(int argc, char **argv)
{
    ...
    XtGetApplicationResources(
        shellWidget, &appData, (XtResourceList)clientResources,
        (Cardinal)XtNumber(clientResources), (ArgList)NULL, (Cardinal)0);
    ...
    XtSetArg(args[3], XtNiconPixmap,
             XCreateBitmapFromData(xDisplay, RootWindow(xDisplay, xScreen),
                                   xtifficon_bits, xtifficon_width,
                                   xtifficon_height));
     ...
     */
    formWidget = XtCreateManagedWidget("form", formWidgetClass, shellWidget,
                                       formArgs, XtNumber(formArgs));

    widget_list[0] = listWidget = XtCreateWidget(
        "list", listWidgetClass, formWidget, listArgs, XtNumber(listArgs));

    widget_list[1] = labelWidget = XtCreateWidget(
        "label", labelWidgetClass, formWidget, labelArgs, XtNumber(labelArgs));

    widget_list[2] = imageWidget = XtCreateWidget(
        "image", widgetClass, formWidget, imageArgs, XtNumber(imageArgs));
    ...
    XChangeWindowAttributes(xDisplay, XtWindow(imageWidget), CWCursor,
                            &window_attributes);
...
void GetTIFFHeader()
{
                      ...
                      &tfPhotometricInterpretation))
    {
        ...
        else if (TIFFGetField(tfFile, TIFFTAG_COLORMAP, &redMap, &greenMap,
                              &blueMap))
        {
    ...
    switch (tfPhotometricInterpretation)
    {
        ...
        case PHOTOMETRIC_PALETTE:
            if (!TIFFGetField(tfFile, TIFFTAG_COLORMAP, &redMap, &greenMap,
                              &blueMap))
            {
                redMap = (uint16_t *)malloc(colormapSize * sizeof(uint16_t));
                greenMap = (uint16_t *)malloc(colormapSize * sizeof(uint16_t));
...
static char *classNames[] = {"StaticGray",  "GrayScale", "StaticColor",
...
void GetVisual()
{
    ...
    switch (tfImageDepth)
    {
        ...
        case 2:
            /*
             * We assume that systems with 24-bit visuals also have 8-bit
             * visuals. We don't promote from 8-bit PseudoColor to 24/32 bit
             * DirectColor.
             */
            switch (tfPhotometricInterpretation)
            {
                case PHOTOMETRIC_MINISWHITE:
                case PHOTOMETRIC_MINISBLACK:
                    if (SearchVisualList((int)tfImageDepth, GrayScale,
                                         &xVisual) == True)
                        break;
                case PHOTOMETRIC_PALETTE:
                    if (SearchVisualList((int)tfImageDepth, PseudoColor,
                                         &xVisual) == True)
                        break;
                default:
                    fprintf(stderr,
            ...
            if (tfImageDepth == 8)
                xColormap = XCreateColormap(
                    xDisplay, RootWindow(xDisplay, xScreen), xVisual, AllocAll);
            else
            {
                xColormap =
                    XCreateColormap(xDisplay, RootWindow(xDisplay, xScreen),
                                    xVisual, AllocNone);
                pixels = (unsigned long *)malloc(colormapSize *
                                                 sizeof(unsigned long));
                MCHECK(pixels);
                (void)XAllocColorCells(xDisplay, xColormap, True, NULL, 0,
                                       pixels, colormapSize);
    ...
    if (appData.verbose == True)
        fprintf(stderr, "%s: Using %d-bit %s visual.\n", fileName, xImageDepth,
                classNames[xVisual->class]);
 ...
 */
Boolean SearchVisualList(image_depth, visual_class, visual)
...
void GetTIFFImage()
{
    ...
    if ((tfImageDepth == 32) || (tfImageDepth == 24))
    {
        output_p = imageMemory =
            (char *)malloc(tfImageWidth * tfImageHeight * 4);
        MCHECK(imageMemory);
        ...
        { /* DECstation 5000 */
            red_shift = pixel_map[0] =
                xRedMask == 0xFF000000
                    ? 3
                    : (xRedMask == 0xFF0000 ? 2 : (xRedMask == 0xFF00 ? 1 : 0));
            green_shift = pixel_map[1] =
                xGreenMask == 0xFF000000
                    ? 3
                    : (xGreenMask == 0xFF0000 ? 2
                                              : (xGreenMask == 0xFF00 ? 1 : 0));
            blue_shift = pixel_map[2] =
                xBlueMask == 0xFF000000
                    ? 3
                    : (xBlueMask == 0xFF0000 ? 2
                                             : (xBlueMask == 0xFF00 ? 1 : 0));
        }
        else
        { /* Ardent */
            red_shift = pixel_map[0] =
            ...
            green_shift = pixel_map[0] =
                xGreenMask == 0xFF000000
                    ? 0
                    : (xGreenMask == 0xFF0000 ? 1
                                              : (xGreenMask == 0xFF00 ? 2 : 3));
            blue_shift = pixel_map[0] =
                xBlueMask == 0xFF000000
                    ? 0
                    : (xBlueMask == 0xFF0000 ? 1
                                             : (xBlueMask == 0xFF00 ? 2 : 3));
        }
        ...
        else
        {
            for (s = 0; s < tfSamplesPerPixel; s++)
            {
                ...
                for (i = 0; i < tfImageHeight; i++)
                {
                    ...
                    input_p = scan_line;
                    output_p =
                        imageMemory + (i * tfImageWidth * 4) + pixel_map[s];
    ...
    else
    {
        ...
        {
            output_p = imageMemory =
                (char *)malloc(tfBytesPerRow * tfImageHeight);
            MCHECK(imageMemory);
        ...
        {
            output_p = imageMemory =
                (char *)malloc(tfBytesPerRow * 2 * tfImageHeight + 2);
            MCHECK(imageMemory);
        ...
        {
            output_p = imageMemory =
                (char *)malloc(tfBytesPerRow * 4 * tfImageHeight + 4);
            MCHECK(imageMemory);
        ...
        else if ((xImageDepth == 4) && (tfImageDepth == 2))
        {
            ...
            for (i = 0; i < tfImageHeight; i++)
            {
                ...
                {
                    *output_p++ =
                        (((*input_p >> 6) << 4) | ((*input_p >> 4) & 3)) +
                        basePixel;
                    *output_p++ =
                        ((((*input_p >> 2) & 3) << 4) | (*input_p & 3)) +
                        basePixel;
                }
            }
        }
...
void CreateXImage()
{
    ...
    {
        xImagePixmap =
            XCreatePixmap(xDisplay, RootWindow(xDisplay, xScreen),
        ...
        {                             /* just pass the bits through */
            ...
            XPutImage(xDisplay, xImagePixmap, bitmap_gc, xImage, 0, 0, 0, 0,
                      xImage->width, xImage->height);
        }
        else
            XPutImage(xDisplay, xImagePixmap, xWinGc, xImage, 0, 0, 0, 0,
                      xImage->width, xImage->height);
...
XtCallbackProc SelectProc(w, unused_1, unused_2)
Widget w;
caddr_t unused_1;
caddr_t unused_2;
{
...
void QuitProc(void) { exit(0); }

void NextProc() { PageProc(ButtonNextPage); }

void PreviousProc() { PageProc(ButtonPreviousPage); }

void PageProc(direction) int direction;
{
    XEvent fake_event;
    Arg args[4];
...
void EventProc(widget, unused, event) Widget widget;
caddr_t unused;
XEvent *event;
{
    ...
    switch (event->type)
    {
        ...
        case KeyPress:
            ...
            switch (XLookupKeysym((XKeyEvent *)event, /* KeySyms index */ 0))
            {
                case XK_Up:
                    if (ih <
                        wh) /* Don't scroll if the window fits the image. */
                        return;
                    sy = yOffset + appData.translate;
                    sy = MIN(ih - wh, sy);
                    if (sy ==
                        yOffset) /* Filter redundant stationary refreshes. */
                        return;
                    yOffset = sy;
                    sx = xOffset;
                    dx = dy = 0;
    ...
    {
        if (xImageDepth == 1)
            XCopyPlane(xDisplay, xImagePixmap, XtWindow(widget), xWinGc, sx, sy,
                       w, h, dx, dy, 1);
        else
            XCopyArea(xDisplay, xImagePixmap, XtWindow(widget), xWinGc, sx, sy,
                      w, h, dx, dy);
    }
    else
        XPutImage(xDisplay, XtWindow(widget), xWinGc, xImage, sx, sy, dx, dy, w,
                  h);
}
...
int XTiffErrorHandler(display, error_event)
Display *display;
