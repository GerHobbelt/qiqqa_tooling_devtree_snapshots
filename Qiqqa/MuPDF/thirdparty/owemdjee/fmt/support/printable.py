...
def print_singletons(uppers, lowers, uppersname, lowersname):
    ...
    for i in range(0, len(lowers), 8):
        print("    {}".format(" ".join("{:#04x},".format(l)
              for l in lowers[i:i+8])))
    print("  };")
...
""")
    print_singletons(singletons0u, singletons0l,
                     'singletons0', 'singletons0_lower')
    print_singletons(singletons1u, singletons1l,
...
""")
    for a, b in extra:
        print(
            "  if (0x{:x} <= cp && cp < 0x{:x}) return false;".format(a, a + b))
    print("""\
  return cp < 0x{:x};
}}\
