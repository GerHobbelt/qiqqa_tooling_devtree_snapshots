...
/* Include warpapp.h *AFTER* the UNICODE defines */
#include "warpapp.h"
...
void winwarn(warpapp_t *app, char *msg)
{
	MessageBoxA(hwndframe, msg, "MuPDF: Warning", MB_ICONWARNING);
}

void winerror(warpapp_t *app, char *msg)
{
	MessageBoxA(hwndframe, msg, "MuPDF: Error", MB_ICONERROR);
	exit(1);
}

void winalert(warpapp_t *app, pdf_alert_event *alert)
{
	int buttons = MB_OK;
...
int winsavequery(warpapp_t *app)
{
...
int winquery(warpapp_t *app, const char *query)
{
...
int wingetcertpath(warpapp_t *app, char *buf, int len)
{
	...
	if (GetOpenFileNameW(&ofn))
	{
		...
		if (code == 0)
		{
			warpapp_error(app, "cannot convert filename to utf-8");
			return 0;
		}
...
int wingetsavepath(warpapp_t *app, char *buf, int len)
{
	...
	ofn.nMaxFile = PATH_MAX;
	ofn.lpstrInitialDir = NULL;
	ofn.lpstrTitle = L"MuPDF: Save PNG file";
	ofn.lpstrFilter = L"PNG files (*.png)\0*.pdf\0All Files\0*\0\0";
...
void winreplacefile(warpapp_t *app, char *source, char *target)
{
	...
	if (sz == 0)
	{
		warpapp_error(app, "cannot convert filename to Unicode");
		return;
	}
...
void wincopyfile(warpapp_t *app, char *source, char *target)
{
	wchar_t wsource[PATH_MAX];
	wchar_t wtarget[PATH_MAX];
...
char *winpassword(warpapp_t *app, char *filename)
{
	char buf[1024], *s;
	int code;
	...
	if (code <= 0)
		warpapp_error(app, "cannot create password dialog");
	if (pd_okay)
		return pd_password;
	return NULL;
}

char *wintextinput(warpapp_t *app, char *inittext, int retry)
{
	int code;
	td_retry = retry;
	...
	if (code <= 0)
		warpapp_error(app, "cannot create text input dialog");
	if (pd_okay)
		return td_textinput;
	return NULL;
}

int winchoiceinput(warpapp_t *app, int nopts, const char *opts[], int *nvals, const char *vals[])
{
	int code;
	cd_nopts = nopts;
	cd_nvals = nvals;
...
static void info()
{
	...
	if (code <= 0)
		warpapp_error(&gapp, "cannot create info dialog");
}
...
dlogaboutproc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_INITDIALOG:
		SetDlgItemTextA(hwnd, 2, warpapp_version(&gapp));
		SetDlgItemTextA(hwnd, 3, warpapp_usage(&gapp));
		return TRUE;
	case WM_COMMAND:
		EndDialog(hwnd, 1);
		return TRUE;
...
void winhelp(warpapp_t*app)
{
	int code = DialogBoxW(NULL, L"IDD_DLOGABOUT", hwndframe, dlogaboutproc);
	if (code <= 0)
		warpapp_error(&gapp, "cannot create help dialog");
}
...
static void winopen()
{
	...
	a = RegisterClassW(&wc);
	if (!a)
		warpapp_error(&gapp, "cannot register frame window class");
	...
	a = RegisterClassW(&wc);
	if (!a)
		warpapp_error(&gapp, "cannot register view window class");
	...
	if (!hwndframe)
		warpapp_error(&gapp, "cannot create frame");
	...
	hwndframe, 0, 0, 0);
	if (!hwndview)
		warpapp_error(&gapp, "cannot create view");
...
void winclose(warpapp_t *app)
{
	if (warpapp_preclose(app))
	{
		do_close(app);
		exit(0);
	}
...
void wincursor(warpapp_t *app, int curs)
{
	if (curs == ARROW)
		SetCursor(arrowcurs);
	if (curs == HAND)
...
void wintitle(warpapp_t *app, char *title)
{
	wchar_t wide[256], *dp;
	char *sp;
	int rune;
...
static void windrawrect(warpapp_t *app, int x0, int y0, int x1, int y1)
{
	RECT r;
	r.left = x0;
	r.top = y0;
...
void windrawstring(warpapp_t *app, int x, int y, char *s)
{
}
...
void winresize(warpapp_t *app, int w, int h)
{
	...
	/* Super crappy hack. No idea what this is about. */
	w += 8;
	h += 8;
...
void winrepaint(warpapp_t *app)
{
	InvalidateRect(hwndview, NULL, 0);
}

void winrepaintsearch(warpapp_t *app)
{
...
void winfullscreen(warpapp_t *app, int state)
{
...
void windocopy(warpapp_t *app)
{
	...
	warpapp_oncopy(&gapp, ucsbuf, 4096);
	GlobalUnlock(handle);
...
void winreloadpage(warpapp_t *app)
{
	SendMessage(hwndview, WM_APP, 0, 0);
}

void winopenuri(warpapp_t *app, char *buf)
{
...
static void handlemouse(int x, int y, int btn, int state)
{
	...
	warpapp_onmouse(&gapp, x, y, btn, modifier, state);
}
...
viewproc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		if (wParam == SIZE_MINIMIZED)
			return 0;
		warpapp_onresize(&gapp, LOWORD(lParam), HIWORD(lParam));
		break;
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	warpapp_setresolution(&gapp, displayRes);
	...
	else
	{
		...
		if (code == 0)
			warpapp_error(&gapp, "cannot convert filename to utf-8");
	}

	warpapp_open(&gapp, filename, 0);
