...
def make_internal_functions( namespace, out_h, out_cpp):
    ...
    out_h.write(
            ...
            template< typename T>
            struct internal_stream_if_state
            {{
                internal_stream_if_state( bool doit, const T& t) : doit(doit), t(t) {{}}
                const bool doit;
                const T& t;
            }};
            template< typename T>
            std::ostream& operator<< (std::ostream& out, const internal_stream_if_state<T>& isi)
            {{
                if (isi.doit) out << isi.t;
                return out;
            }}
            template< typename T>
            internal_stream_if_state< T> internal_stream_if( bool doit, const T& t)
            {{
                return internal_stream_if_state<T>( doit, t);
            }}
