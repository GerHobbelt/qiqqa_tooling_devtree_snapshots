...
static int
push_list(fz_context *ctx, mark_list *list, pdf_obj *obj)
{
...
static void
pop_list(fz_context *ctx, mark_list *list)
{
	pdf_unmark_obj(ctx, list->list[--list->len]);
}

static void
free_list(fz_context *ctx, mark_list *list)
{
	while (list->len)
		pop_list(ctx, list);
...
static void copystream(fz_context *ctx, pdf_document *doc, pdf_write_state *opts, pdf_obj *obj_orig, int num, int gen, int do_deflate, int unenc)
{
	...
	fz_try(ctx)
	{
		...
		if (do_deflate && !pdf_dict_get(ctx, obj, PDF_NAME(Filter)))
		{
			...
			{
				buf = tmp_comp = fz_compress_ccitt_fax_g4(ctx, buf->data, w, h);
...
const char *fz_pdf_write_options_usage =
	...
	"  decrypt:           write decrypted document (otherwise keep encryption as-is)\n"
	"  encrypt=keep|rc4-40(default)|rc4-128|aes-128|aes-256:\n"
