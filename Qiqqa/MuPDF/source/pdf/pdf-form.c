...
static pdf_obj *find_head_of_field_group(fz_context *ctx, pdf_obj *obj)
{
	if (!pdf_is_dict(ctx, obj))
		return NULL;
	else if (pdf_dict_get(ctx, obj, PDF_NAME(T)))
		return obj;
	else
...
lookup_field_sub(fz_context *ctx, pdf_obj *dict, const char *str, pdf_cycle_list *cycle_up)
{
	...
	if (pdf_is_array(ctx, kids) && *str != 0)
...
static void *find_widget_on_page(fz_context *ctx, fz_page *page_, void *state_)
{
	...
	if (pdf_is_dict(ctx, state->pageobj) && pdf_objcmp_resolve(ctx, state->pageobj, page->obj))
		return NULL;
...
pdf_obj *pdf_button_field_on_state(fz_context *ctx, pdf_obj *field)
{
	...
	if (!pdf_is_name(ctx, on)) on = find_on_state(ctx, pdf_dict_get(ctx, ap, PDF_NAME(D)));
	if (!pdf_is_name(ctx, on)) on = PDF_NAME(Yes);
	return on;
}
...
static void toggle_check_box(fz_context *ctx, pdf_annot *annot)
{
	...
	fz_try(ctx)
	{
		...
		if (pdf_is_name(ctx, as) && as != PDF_NAME(Off))
		{
...
enum pdf_widget_tx_format pdf_text_widget_format(fz_context *ctx, pdf_annot *tw)
{
	...
	if (pdf_is_string(ctx, js) || pdf_is_stream(ctx, js))
	{
...
const char *pdf_choice_field_option(fz_context *ctx, pdf_obj *field, int _export, int i)
{
	pdf_obj *opt = pdf_dict_get_inheritable(ctx, field, PDF_NAME(Opt));
	pdf_obj *ent = pdf_array_get(ctx, opt, i);
	if (pdf_array_len(ctx, ent) == 2)
		return pdf_array_get_text_string(ctx, ent, _export ? 0 : 1);
	else
...
int pdf_signature_is_signed(fz_context *ctx, pdf_document *doc, pdf_obj *field)
{
	...
	return pdf_is_dict(ctx, v) && (pdf_is_name(ctx, vtype) ? pdf_name_eq(ctx, vtype, PDF_NAME(Sig)) : 1);
}
...
{
	if (widget->is_being_edited != editing)
...
static void pdf_execute_action_chain(fz_context *ctx, pdf_document *doc, pdf_obj *target, const char *path, pdf_obj *action, pdf_cycle_list *cycle_up)
{
	...
	else
	{
		...
		if (pdf_is_dict(ctx, next) || pdf_is_array(ctx, next))
