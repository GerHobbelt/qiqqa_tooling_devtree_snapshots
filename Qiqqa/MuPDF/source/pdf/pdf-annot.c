...
static void check_allowed_subtypes(fz_context *ctx, pdf_annot *annot, pdf_obj *property, const pdf_obj **allowed)
{
	...
	if (!is_allowed_subtype(ctx, annot, property, allowed))
		fz_throw(ctx, FZ_ERROR_ARGUMENT, "%s annotations have no %s property", pdf_to_name_not_null(ctx, subtype), pdf_to_name_not_null(ctx, property));
}
...
pdf_create_link(fz_context *ctx, pdf_page *page, fz_rect bbox, const char *uri)
{
	...
	fz_try(ctx)
	{
		...
		*/
		...
		link = pdf_new_link(ctx, page, bbox, 0, NULL, uri, annot_obj);
...
remove_from_tree(fz_context *ctx, pdf_obj *arr, pdf_obj *item, pdf_cycle_list *cycle_up)
{
	...
	{
		fz_throw(ctx, FZ_ERROR_GENERIC, "PDF file format: expected array type while removing (annotation) item from tree. Cannot remove element.");
		return 0;
	}
...
void
pdf_annot_text_color(fz_context *ctx, pdf_annot *annot, int *n, float color[4])
{
	do_pdf_annot_color(ctx, annot, n, color, PDF_NAME(TextColor));
}
