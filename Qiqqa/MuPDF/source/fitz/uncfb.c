...
static void
readbuf(fz_context *ctx, fz_stream *stm, uint8_t *buf, size_t size)
{
...
read_fat(fz_context *ctx, fz_cfb_archive *cfb, uint32_t sector)
{
	...
	if (real_sect != cfb->fatcache_sector)
	{
		...
		readbuf(ctx, cfb->super.file, &cfb->fatcache[0], sector_size);
...
read_mini_fat(fz_context *ctx, fz_cfb_archive *cfb, uint32_t sector)
{
	...
	if (!cache_valid)
	{
		...
		readbuf(ctx, cfb->super.file, cfb->minifatcache, sector_size);
	}
...
cfb_next(fz_context *ctx, fz_stream *stm, size_t required)
{
	...
	readbuf(ctx, cfb->super.file, state->buffer, sector_size);
	stm->rp = state->buffer;
...
cfb_next_mini(fz_context *ctx, fz_stream *stm, size_t required)
{
	...
	readbuf(ctx, state->mini_stream, state->buffer, MINI_SECTOR_SIZE);
	stm->rp = state->buffer;
