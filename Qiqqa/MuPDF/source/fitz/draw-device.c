		...
		for (i = 0; i < span->len; i++)
		{
			...
			if (glyph)
			{
				...
				else
				{
					...
					fz_paint_image(ctx, state->dest, &state->scissor, state->shape, state->group_alpha, pixmap, mat, alpha * 255 + 0.5f, !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES), eop);
				}
...
fz_draw_fill_image(fz_context *ctx, fz_device *devp, fz_image *image, fz_matrix in_ctm, float alpha, fz_color_params color_params)
{
	...
	fz_try(ctx)
	{
		...
		fz_paint_image(ctx, state->dest, &state->scissor, state->shape, state->group_alpha, pixmap, local_ctm, alpha * 255 + 0.5f, !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES), eop);
...
fz_draw_begin_group(fz_context *ctx, fz_device *devp, fz_rect area, fz_colorspace *cs, int isolated, int knockout, int blendmode, float alpha)
{
	...
	{
		state[1].group_alpha = NULL;
		state[1].alpha = alpha || state[0].dest->alpha || isolated;

		//state[1].scissor = bbox;
		state[1].blendmode = blendmode | (knockout ? FZ_BLEND_KNOCKOUT : 0);
	}
