...
void
fz_save_pixmap_as_tiff(fz_context *ctx, const fz_pixmap *pixmap, const char *filename)
{
...
typedef struct tiff_band_writer_s
{
...
static void
tiff_write_icc(fz_context *ctx, tiff_band_writer *writer, fz_colorspace *cs)
{
#if FZ_ENABLE_ICC
...
static void
tiff_write_header(fz_context *ctx, fz_band_writer *writer_, fz_colorspace *cs)
{
	...
	static const unsigned char tiffsig[8] = { 137, 80, 78, 71, 13, 10, 26, 10 };
	...
	fz_write_data(ctx, out, tiffsig, 8);
	...
	tiff_write_icc(ctx, writer, cs);
}

static int tiff_compresion_level = Z_DEFAULT_COMPRESSION;

void fz_default_tiff_compression_level(int level)
{
	if (level < 0 || level > Z_BEST_COMPRESSION)
		level = Z_DEFAULT_COMPRESSION;
	tiff_compresion_level = level;
}

static void
tiff_write_band(fz_context *ctx, fz_band_writer *writer_, int stride, int band_start, int band_height, const unsigned char *sp)
{
	tiff_band_writer *writer = (tiff_band_writer *)(void *)writer_;
	fz_output *out = writer->super.out;
	unsigned char *dp;
...
static void
tiff_write_trailer(fz_context *ctx, fz_band_writer *writer_)
{
	tiff_band_writer *writer = (tiff_band_writer *)(void *)writer_;
	fz_output *out = writer->super.out;
...
static void
tiff_drop_band_writer(fz_context *ctx, fz_band_writer *writer_)
{
	tiff_band_writer *writer = (tiff_band_writer *)(void *)writer_;
...
fz_band_writer *fz_new_tiff_band_writer(fz_context *ctx, fz_output *out)
{
	...
	writer->super.header = tiff_write_header;
...
/* We use an auxiliary function to do pixmap_as_tiff, as it can enable us to
...
static fz_buffer *
tiff_from_pixmap(fz_context *ctx, fz_pixmap *pix, fz_color_params color_params, int drop)
{
	fz_buffer *buf = NULL;
	fz_output *out = NULL;
	fz_pixmap *pix2 = NULL;
...
#pragma message("TODO: throw exception in strict mode. Also check out 'ignore_errors' in mudraw tool and link this to that setting.")
	...
	fz_try(ctx)
	{
		...
		fz_write_pixmap_as_tiff(ctx, out, pix);
		fz_close_output(ctx, out);
	}
	fz_always(ctx)
	{
...
fz_buffer *
fz_new_buffer_from_image_as_tiff(fz_context *ctx, const fz_image *image, fz_color_params color_params)
{
	fz_pixmap *pix = fz_get_pixmap_from_image(ctx, image, NULL, NULL, NULL, NULL);
	return tiff_from_pixmap(ctx, pix, color_params, 1);
}

fz_buffer *
fz_new_buffer_from_pixmap_as_tiff(fz_context *ctx, const fz_pixmap *pix, fz_color_params color_params)
{
