...
fz_debug_html_flow(fz_context *ctx, fz_output* out, fz_html_flow *flow, int level)
{
	...
	while (flow)
	{
		if (flow->box != sbox) {
			...
			indent(level);
#ifndef NDEBUG
			fz_write_printf(ctx, out, "@style <%s> em=%g font='%s'", sbox->tag, sbox->s.layout.em, fz_font_name(ctx, sbox->style->font));
#else
			fz_write_printf(ctx, out, "@style em=%g font='%s'", sbox->s.layout.em, fz_font_name(ctx, sbox->style->font));
#endif
...
fz_debug_html_box(fz_context *ctx, fz_output *out, fz_html_box *box, int level)
{
	while (box)
	{
		...
		if (box->list_item)
			fz_write_printf(ctx, out, " list=%d", box->list_item);
		if (box->id)
			fz_write_printf(ctx, out, " id=(%s)", box->id);
		if (box->href)
